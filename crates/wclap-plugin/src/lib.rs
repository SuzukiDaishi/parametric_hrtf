...
// The exported `#[no_mangle] malloc` is gated to wasm32: on a native test/host
// build it would override libc's `malloc` (wrong signature → heap corruption /
// SIGSEGV). The implementation lives in the un-exported `malloc_impl` so the
// rest of the ABI code (`factory_create_plugin`) compiles on both targets.
...
extern "C" fn factory_create_plugin(_factory: u32, host: u32, plugin_id_ptr: u32) -> u32 {
    ...
    let plugin_ptr = malloc_impl(offsets::plugin::SIZE as u32);
    if plugin_ptr == 0 {
        return 0;
    }
