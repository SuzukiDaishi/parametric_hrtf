// `wclap-host-js` scans the plugin's exports for a WebAssembly.Table to use as
// the function-table source, then grows it at runtime to install host
...
// (Mirrors z-audio-dsp-plugin's WebCLAP crates.)
fn main() {
